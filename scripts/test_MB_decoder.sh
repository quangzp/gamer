...
if [ "${backbone}" = "Qwen3Moe" ]; then
    backbone_arg=Qwen3Moe
...
if [ $rq_kmeans -eq 0 ]; then
    ...
    if [ $cid -eq 0 ]; then
        ...
        if [ $rid -eq 0 ]; then
            ...
            if [ $original -eq 0 ]; then
                ...
                ckpt_path=./checkpoint/MB-decoder/${task_dir}/alpha${alpha}-beta${beta}/
                index_file=.index.epoch${epoch}.alpha${alpha}-beta${beta}.json
                echo "Testing MB decoder on ${dataset} using RQ-VAE with alpha=${alpha}, beta=${beta}, epoch=${epoch} using GPU ${gpu}."
            else
                results_file=./results/${task_dir}/results-${test_task}-original.json
                ckpt_path=./checkpoint/MB-decoder/${task_dir}/original/
                index_file=.index.json
                echo "Testing MB decoder on ${dataset} using original index file from LETTER repository."
            fi
        else
            results_file=./results/${task_dir}/results-${test_task}-rid.json
            ckpt_path=./checkpoint/MB-decoder/${task_dir}/rid/
            index_file=.index.rid.json
            echo "Testing MB decoder on ${dataset} using random ID tokenization."
        fi
    else
        : ${chunk_size:=64}
        : ${shuffle:=0}
        if [ $shuffle -eq 1 ]; then
            ...
            ckpt_path=./checkpoint/MB-decoder/${task_dir}/cid-shuffle-${chunk_size}/
            index_file=.index.cid.shuffle.chunk${chunk_size}.json
            echo "Testing MB decoder on ${dataset} using chunked ID tokenization with chunk size ${chunk_size} and shuffling."
        else
            results_file=./results/${task_dir}/results-${test_task}-cid-${chunk_size}.json
            ckpt_path=./checkpoint/MB-decoder/${task_dir}/cid-${chunk_size}/
            index_file=.index.cid.chunk${chunk_size}.json
            echo "Testing MB decoder on ${dataset} using chunked ID tokenization with chunk size ${chunk_size}."
        fi
    fi
else
    : ${cf_emb:=0}
    if [ $cf_emb -eq 0 ]; then
        ...
        ckpt_path=./checkpoint/MB-decoder/${task_dir}/rq-kmeans/
        index_file=.index.rq-kmeans.json
        echo "Testing MB decoder on ${dataset} using RQ-Kmeans without CF embeddings."
    else
        : ${reduce:=0}
        if [ $reduce -eq 0 ]; then
            results_file=./results/${task_dir}/results-${test_task}-rq-kmeans-cf.json
            ckpt_path=./checkpoint/MB-decoder/${task_dir}/rq-kmeans-cf/
            index_file=.index.rq-kmeans-cf.json
            echo "Testing MB decoder on ${dataset} using RQ-Kmeans with CF embeddings."
        else
            results_file=./results/${task_dir}/results-${test_task}-rq-kmeans-cf-reduce.json
            ckpt_path=./checkpoint/MB-decoder/${task_dir}/rq-kmeans-cf-reduce/
            index_file=.index.rq-kmeans-cf-reduce.json
            echo "Testing MB decoder on ${dataset} using RQ-Kmeans with CF embeddings and reduced semantic embeddings."
        fi
    fi
fi
...
if [ $gpu_num -eq 1 ]; then
    ...
    python main.py test_MB_decoder \
    ...
    torchrun --nproc_per_node=${gpu_num} --master_port=${port} ./main.py test_MB_decoder \
        --backbone ${backbone_arg} \
        --ckpt_path ${ckpt_path} \
        --dataset ${dataset} \
        --data_path ${data_path} \
